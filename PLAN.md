...
| **Move the e2e suite onto a production build.** It runs against `next dev` today because sheet, modal and filter menu have no caller outside `/dev/kitchen-sink`, and `pageExtensions` keeps that page out of a build entirely — so most of the suite would have nothing to drive. The cost of leaving it is worse than the dev server being slow: the suite goes on asserting against the mock while the real thing ships uncovered, and it stays green the whole time. Moving it means pointing `playwright.config.ts` at `next build && next start` and repointing `overlays.spec.ts` at the real filter row and the real modal.                                                                                                                                                                                                                                                                           | Phases 4–5, with the first real filter row or modal |
